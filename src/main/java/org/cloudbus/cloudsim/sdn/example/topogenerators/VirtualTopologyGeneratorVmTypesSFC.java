...
public class VirtualTopologyGeneratorVmTypesSFC extends VirtualTopologyGeneratorVmTypes {
	...
	public static void main(String [] argv) {
		...
		String fileName = "sfc.virtual.min.json";
		boolean isMinMode = true;
		if (Objects.equals(argv[0], "max")) {
			noscale = true;
			fileName = "sfc.virtual.max.json";
			isMinMode = false;
		}
		vmGenerator.generateLarge3TierTopologySFC(fileName, noscale, isMinMode);
	}

	public void generateLarge3TierTopologySFC(String jsonFileName, boolean noscale, boolean isMinMode) {
		final int numWeb=8;
		final int numApp=24;
		final int numDB=2;
		...
		for(int vmGroupId = 0;vmGroupId < groupNum; vmGroupId++) {
			...
			generateVMGroupComplex(numWeb, numApp, numDB, startTime, endTime, linkBW.get(vmGroupId), vmGroupId, noscale);
		}
